...
SAMPLE_CASES = [
    ...
    ("sample8_lexical_error", "测试组 8：词法错误——非法字符（sample8_lexical_error）"),
]
...
def parse_exp1_from_output(txt_path: Path) -> tuple[list[str], list[str], list[str]]:
    """从 docs/test-output/*.txt 解析「实验一」二元组、符号表与词法错误（至实验二之前）。"""
    tokens: list[str] = []
    symbols: list[str] = []
    ...
    for line in read_output_text(txt_path).splitlines():
        ...
        if line.strip() == "--- (单词名称, 单词类别) ---":
            in_tokens, in_symbols, in_errors = True, False, False
            continue
        if line.strip() == "--- 符号表（按出现顺序） ---":
            in_tokens, in_symbols, in_errors = False, True, False
            continue
        if line.strip() == "--- 词法错误 ---":
            in_tokens, in_symbols, in_errors = False, False, True
            continue
        if line.startswith("--- "):
            in_tokens = in_symbols = in_errors = False
            continue
        ...
        if in_errors and line.strip():
            lexical_errors.append(line.strip())
    return tokens, symbols, lexical_errors
...
def load_test_cases_from_exports() -> list[tuple[str, str, list[str], str, str]]:
    ...
    for base, title in SAMPLE_CASES:
        ...
        tokens, symbols, lexical_errors = parse_exp1_from_output(out_file)
...
def build_document() -> Document:
    ...
    add_para(
        doc,
        "以下 8 组数据均来自工程样例源程序 samples/*.src，经 CompilerApplication 运行后由 "
        "scripts/export-test-outputs.ps1 导出；词法部分摘自 docs/test-output/*.txt 中「实验一：词法分析」"
        "章节（与控制台输出一致），未另行编造。",
        indent=True,
    )
    add_para(
